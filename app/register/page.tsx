...
export default function RegisterPage() {
  ...
  // ─── Google OAuth (même logique que login) ───────────────────────────────────

  const handleGoogleCredential = useCallback(async (credential: string) => {
    setError("");
    setLoading(true);
    try {
      // mode register : crée le compte si inexistant, redirige vers dashboard si existant
    ...
    <div className="min-h-screen flex items-center justify-center bg-background-main py-12 px-4 sm:px-6 lg:px-8">
      ...
      >
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold tracking-tight text-text-main">Créer un compte</h2>
          <p className="mt-2 text-sm text-text-muted">Lancez votre programme de fidélité en 2 minutes</p>
        </div>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-white text-text-muted">ou inscrivez-vous avec votre email</span>
          </div>
        </div>
